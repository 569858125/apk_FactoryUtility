...
public class WifiAccessPoint implements Comparable<WifiAccessPoint> {
    static final String TAG = "WifiAccessPoint";
    ...
    private void loadConfig(WifiConfiguration config) {
        Log.d(TAG, "loadConfig ssid " + config.SSID);
    ...
    private void loadResult(ScanResult result) {
        Log.d(TAG, "loadResult ssid " + result.SSID);
        ssid = result.SSID;
        bssid = result.BSSID;
    ...
    @Override
    public int compareTo(WifiAccessPoint accessPoint) {
        WifiAccessPoint other = accessPoint;
    ...
    public boolean update(WifiInfo info, DetailedState state) {
        boolean reorder = false;
        if (info != null && networkId != -1/*WifiConfiguration.INVALID_NETWORK_ID */ && networkId == info.getNetworkId()) {
            reorder = (mInfo == null);
            mRssi = info.getRssi();
            mInfo = info;
            mState = state;
    ...
    public int getLevel() {
        ...
        System.out.println("mRssi " + mRssi);
        return mRssi;
    }
